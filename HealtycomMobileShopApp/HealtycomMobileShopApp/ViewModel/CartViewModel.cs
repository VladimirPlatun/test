...
namespace HealtycomMobileShopApp.ViewModel
{
    public class CartViewModel : BaseViewModel
    {
        ...
        public ICommand AddItem => new Command(async (obj) =>
        {
            ...
            OnPropertyChanged(nameof(item.NumberOfItem));

            await Task.Run(() => CartService.IncreaseQuantityAsync(item.Product.ProductName));
        ...
        public ICommand SubtractItem => new Command(async (obj) =>
        {
            ...
            if (item.NumberOfItem > 0)
            {
                ...
                await Task.Run(() => CartService.DecreaseQuantityAsync(item.Product.ProductName));
